...
export function useShareChannels(url: string, text: string, score?: number) {
...
export function ShareButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex cursor-pointer items-center gap-1.5 rounded-lg bg-surface px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-surface-raised"
    >
      <ShareIcon />
      Share
    </button>
...
export function ShareModal({
  ...
  const subtitle = personaName
    ? `${personaName} finished the critique. Share the results.`
    : "Your audit is ready. Share the results.";
  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          <CloseButton onClick={onClose} className="absolute right-4 top-4 cursor-pointer rounded-full p-2 text-muted transition-colors hover:text-foreground" />
          ...
          >
            <div className="flex flex-col items-center gap-5">
              ...
              <div className="flex flex-col items-center gap-1">
                <p className="text-center text-lg font-semibold text-foreground">Audit complete</p>
                <p className="max-w-xs text-center text-sm text-muted">{subtitle}</p>
              </div>
            ...
            {hasNativeShare && (
              <button
                ...
                className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-full bg-foreground px-6 py-3.5 text-sm font-semibold text-background transition-colors hover:bg-foreground/85"
              >
                <ShareIcon size={16} />
                Share walkthrough
              </button>
            ...
            <div className="grid w-full grid-cols-5 gap-2">
              {channels.map((ch) => {
                const content = (
                  <div className="flex cursor-pointer flex-col items-center gap-1.5 rounded-xl bg-surface p-3 transition-colors hover:bg-surface-raised">
                    <span className="text-foreground">{ch.icon}</span>
                    <span className="text-[10px] font-medium text-foreground/80">{ch.label}</span>
                  </div>
                );
            ...
            {onBackToAudit && (
              <button
                ...
                className="cursor-pointer rounded-full bg-surface px-6 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-surface-raised"
              >
                &larr; Back to audit
              </button>
            )}
